...
class EnDS(object):
    ...
    def prep_for_dsi(self,sim_ensemble=None,t_d="dsi_template",
        ...
        """
        print(warnings.warn("EnDS.prep_for_dsi() is deprecated. Use pyemu.emulators.DSI instead",PyemuWarning))
        if sim_ensemble is None:
            sim_ensemble = self.sim_ensemble.copy()

        from pyemu.emulators import DSI
        dsi = DSI(sim_ensemble=sim_ensemble,energy_threshold=energy);
        transforms=None
        if apply_normal_score_transform:
            transforms = [{"type":"normal_score","quadratic_extrapolation":nst_extrap}]
        dsi.apply_feature_transforms(transforms=transforms)
        dsi.fit()
        
        pst = dsi.prepare_pestpp(t_d)
        dsi.save(os.path.join(t_d,"dsi.pickle"))
        return pst

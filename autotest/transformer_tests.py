...
def test_dsi_feature_transforms():
    ...
    dsi = pyemu.emulators.Emulator.DSI(
        pst=pst,
        sim_ensemble=sim_ensemble,
        log_transform=True,
        normal_score_transform=True
    )
    ...
    dsi2 = pyemu.emulators.Emulator.DSI(
        pst=pst,
        sim_ensemble=sim_ensemble,
        log_transform=sim_names[:2],  # Only transform first two columns
